...
- 作業ディレクトリ: `D:\AI\ChromeExtension\table-product-price-compare-panel`
...
```powershell
cd D:\AI\ChromeExtension\table-product-price-compare-panel
npm test
npm start
```
...
5. Chrome拡張 Side Panel + compare table の UI または配布面を開き、検証結果をユーザーが次アクションに移せる粒度で確認する。
...
- docs ZIP が `dist/table-product-price-compare-panel-docs.zip` に存在する。
